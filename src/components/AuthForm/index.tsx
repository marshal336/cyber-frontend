...
import { Tabs, TabsList, TabsTrigger, TabsContent, Button, Form } from '@/components/ui'
...
export default function AuthForm({ className }: IAuthFormProps) {
    ...
    return (
        <div className={cn(className, 'border rounded-lg border-black flex flex-col items-center p-4')}>
            <h3 className='sm:text-4xl text-xl font-bold'>{variant === 'register' ? 'Create Account' : 'Log in'}</h3>
            <Tabs defaultValue={'register'} className='w-full'>
                <TabsList className='flex gap-3 !bg-transparent'>
                    {values.map((value, i) => (
                        <TabsTrigger
                            className={cn('!bg-transparent rounded-none capitalize sm:text-lg text-md text-black/90', i === index ? 'border-b-[4px] border-b-black' : '')}
                            onClick={() => {
                                setVariant(value)
                                setIndex(i)
                            }}
                            value={variant}>{value}</TabsTrigger>
                    ))}
                </TabsList>
                {values.map((value, i) => (
                        ...
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-2 px-4">
                                ...
                                <div className="flex justify-between">
                                    <Button type="submit" className='max-w-[100px] mt-6 mb-4'>Submit</Button>
